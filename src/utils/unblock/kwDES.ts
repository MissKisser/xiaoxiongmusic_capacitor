/**
 * 酷我音乐 DES 加密算法
 * 移植自 SPlayer/server/unblock/kwDES.ts
 * 使用纯 JavaScript BigInt 实现，无 Node.js 依赖
 */
...
interface LongType {
    ...
    not: () => LongType;
    isNegative: () => boolean;
    or: (x: number | bigint | LongType) => LongType;
    and: (x: number | bigint | LongType) => LongType;
    xor: (x: number | bigint | LongType) => LongType;
    equals: (x: number | bigint | LongType) => boolean;
    multiply: (x: number | bigint | LongType) => LongType;
    shiftLeft: (x: number | bigint) => LongType;
    shiftRight: (x: number | bigint) => LongType;
}

const Long = (n: number | bigint | string): LongType => {
    const bN = BigInt(n);

    return {
        low: Number(bN & BigInt(0xffffffff)),
        valueOf: () => bN.valueOf(),
        toString: () => bN.toString(),
        not: () => Long(~bN),
        isNegative: () => bN < 0,
        or: (x: number | bigint | LongType) => Long(bN | BigInt(typeof x === 'object' ? x.valueOf() : x)),
        and: (x: number | bigint | LongType) => Long(bN & BigInt(typeof x === 'object' ? x.valueOf() : x)),
        xor: (x: number | bigint | LongType) => Long(bN ^ BigInt(typeof x === 'object' ? x.valueOf() : x)),
        equals: (x: number | bigint | LongType) => bN === BigInt(typeof x === 'object' ? x.valueOf() : x),
        multiply: (x: number | bigint | LongType) => Long(bN * BigInt(typeof x === 'object' ? x.valueOf() : x)),
        shiftLeft: (x: number | bigint) => Long(bN << BigInt(x)),
        shiftRight: (x: number | bigint) => Long(bN >> BigInt(x)),
    };
};

const range = (n: number) => Array.from(new Array(n).keys());
const power = (base: number | bigint, index: number) =>
    Array(index)
        .fill(null)
        .reduce((result: LongType) => result.multiply(base), Long(1));
const LongArray = (...array: number[]) => array.map((n) => (n === -1 ? Long(-1) : Long(n)));

// EXPANSION
const arrayE = LongArray(
    31, 0, 1, 2, 3, 4, -1, -1, 3, 4, 5, 6, 7, 8, -1, -1, 7, 8, 9, 10, 11, 12, -1, -1, 11, 12, 13, 14, 15, 16, -1, -1,
...
const bitTransform = (arrInt: LongType[], n: number, l: LongType) => {
    let l2 = Long(0);
    range(n).forEach((i) => {
        const arrIdx = arrInt[i].low;
        if (arrInt[i].isNegative() || l.and(arrayMask[arrIdx]).equals(0)) return;
        l2 = l2.or(arrayMask[i]);
    });
    return l2;
};

const DES64 = (longs: LongType[], l: LongType) => {
    const pR = range(8).map(() => Long(0));
    const pSource: LongType[] = [Long(0), Long(0)];
    let L = Long(0);
    let R = Long(0);
    ...
    range(16).forEach((i) => {
        ...
        range(8)
            .reverse()
            .forEach((sbi) => {
                const pRIdx = Number(pR[sbi].valueOf()) & 0xff;
                SOut = SOut.shiftLeft(4).or(matrixNSBox[sbi][pRIdx]);
            });
        R = bitTransform(arrayP, 32, SOut);
        L = Long(pSource[0].valueOf());
        pSource[0] = Long(pSource[1].valueOf());
        pSource[1] = L.xor(R);
    });
    pSource.reverse();
...
const subKeys = (l: LongType, longs: LongType[], n: number) => {
...
export const encrypt = (msg: Uint8Array) => crypt(msg, SECRET_KEY, 0);
export const decrypt = (msg: Uint8Array) => crypt(msg, SECRET_KEY, 1);

/**
 * 加密查询字符串并返回 Base64 编码
 */
export const encryptQuery = (query: string): string => {
    const msgBytes = new TextEncoder().encode(query);
    const encrypted = encrypt(msgBytes);
    // 手动实现 Base64 编码（避免依赖 Buffer）
