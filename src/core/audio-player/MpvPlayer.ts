...
export class MpvPlayer extends EventTarget implements IPlaybackEngine {
  ...
  private _eqBands: number[] = Array(10).fill(0);
  private eqFlushTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  public destroy(): void {
    ...
    window.electron.ipcRenderer.removeAllListeners("mpv-playback-restart");
    window.electron.ipcRenderer.removeAllListeners("mpv-ended");
    if (this.eqFlushTimer) {
      clearTimeout(this.eqFlushTimer);
      this.eqFlushTimer = null;
    }
  ...
  /**
   * 设置均衡器增益（10 段）
   * 由于 UI 会频繁调用，这里做 debounce，最终一次性把 10 段下发给主进程
   */
  public setFilterGain(index: number, value: number): void {
    if (index < 0 || index >= 10) return;
    const v = Number.isFinite(value) ? Math.max(-12, Math.min(12, value)) : 0;
    this._eqBands[index] = v;

    if (!window?.electron?.ipcRenderer) return;
    if (this.eqFlushTimer) clearTimeout(this.eqFlushTimer);
    this.eqFlushTimer = setTimeout(() => {
      this.eqFlushTimer = null;
      window.electron.ipcRenderer.send("mpv-set-eq", [...this._eqBands]);
    }, 60);
  }
