...
// 检查路由名称是否是有效的菜单项
const isValidMenuKey = (key: string, options: MenuOption[] | MenuGroupOption[]): boolean => {
  for (const opt of options) {
    if ('key' in opt && opt.key === key) return true;
    if ('children' in opt && opt.children) {
      if (isValidMenuKey(key, opt.children)) return true;
    }
  }
  return false;
};
...
const checkMenuItem = () => {
  ...
  switch (routerName) {
    ...
    case "desktop-lyric":
    case "search":
      // 这些页面不应该改变菜单选中状态，保持当前选中
      break;
    default:
      // 只有当 routerName 是有效的菜单项时才更新
      if (isValidMenuKey(routerName, menuOptions.value)) {
