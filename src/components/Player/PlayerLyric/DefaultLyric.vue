<template>
  <div
    ...
    :style="{
      '--lrc-size': displayFontSize.main + 'px',
      '--lrc-tran-size': displayFontSize.tran + 'px',
      '--lrc-roma-size': displayFontSize.roma + 'px',
...
const displayFontSize = computed(() => {
  let main = settingStore.lyricFontSize;
  let tran = settingStore.lyricTranFontSize;
  let roma = settingStore.lyricRomaFontSize;

  // 如果是移动端，且当前值还是默认值（46），则自动缩减到移动端推荐值（32）
  if (isMobile.value && main === 46) {
    main = 32;
    tran = 18;
    roma = 14;
  }

  return { main, tran, roma };
});
...
.lyric {
  ...
  .countdown-line {
    ...
    .count-down {
      ...
      .point {
        width: calc(var(--lrc-size) * 0.6);
        height: calc(var(--lrc-size) * 0.6);
        margin-right: calc(var(--lrc-size) * 0.25);
        border-radius: 50%;
