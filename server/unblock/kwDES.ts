...
const Long = (n: any) => {
  ...
  return {
    ...
    not: () => Long(~bN),
    isNegative: () => bN < 0,
    or: (x: any) => Long(bN | BigInt(x)),
    and: (x: any) => Long(bN & BigInt(x)),
    xor: (x: any) => Long(bN ^ BigInt(x)),
    equals: (x: any) => bN === BigInt(x),
    multiply: (x: any) => Long(bN * BigInt(x)),
    shiftLeft: (x: any) => Long(bN << BigInt(x)),
    shiftRight: (x: any) => Long(bN >> BigInt(x)),
  };
};

const range = (n: number) => Array.from(new Array(n).keys());
const power = (base: any, index: number) =>
  Array(index)
    .fill(null)
    .reduce((result: any) => result.multiply(base), Long(1));
...
const bitTransform = (arrInt: any[], n: number, l: any) => {
  let l2 = Long(0);
  range(n).forEach((i) => {
...
const subKeys = (l: any, longs: any[], n: number) => {
...
const crypt = (msg: Buffer, key: Buffer, mode: number) => {
  let l = Long(0);
  range(8).forEach((i) => {
    l = Long(key[i])
      .shiftLeft(i * 8)
...
export const encrypt = (msg: Buffer) => crypt(msg, SECRET_KEY, 0);
export const decrypt = (msg: Buffer) => crypt(msg, SECRET_KEY, 1);
export const encryptQuery = (query: string) => encrypt(Buffer.from(query)).toString("base64");
